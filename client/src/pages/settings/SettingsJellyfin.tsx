...
interface AddServerModalProps {
  onClose: () => void;
  onAdded: (server: JellyfinServer) => void;
}

const SettingsJellyfin: FC = () => {
  const { user } = useAuth();
  const { addToast } = useToast();
  const [jellyfinServers, setJellyfinServers] = useState<JellyfinServer[]>([]);
  ...
  const loadJellyfinServers = async (): Promise<void> => {
    try {
      const servers = await api.getJellyfinServers();
      setJellyfinServers(servers);
    } catch (err) {
      console.error('Failed to load Jellyfin servers:', err);
    }
  };

  const loadSyncLogs = async (): Promise<void> => {
    try {
      const logs = await api.getJellyfinSyncLogs(10);
      setSyncLogs(logs);
    } catch (err) {
  ...
  const syncAllToJellyfin = async (): Promise<void> => {
    setLoading(prev => ({ ...prev, sync: true }));
    setSyncResult(null);
    try {
      const result = await api.syncToJellyfin();
  ...
  const syncServerToJellyfin = async (serverId: string): Promise<void> => {
    setLoading(prev => ({ ...prev, [`sync-${serverId}`]: true }));
    try {
      const result = await api.syncToJellyfinServer(serverId);
      if (result.success) {
  ...
  const deleteJellyfinServer = async (id: string): Promise<void> => {
    try {
      await api.deleteJellyfinServer(id);
      setJellyfinServers(jellyfinServers.filter((s) => s.id !== id));
      addToast('Jellyfin server deleted successfully', 'success');
    } catch (err: any) {
    ...
    <div className="animate-fade-in space-y-6">
      ...
      <div className="bg-card border border-border/50 rounded-xl p-6">
        <div className="flex items-center justify-between mb-6">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-lg font-semibold m-0">Jellyfin Servers</h2>
              <p className="text-sm text-muted-foreground m-0">Manage your connected Jellyfin servers</p>
            </div>
          </div>
        ...
        {jellyfinServers.length === 0 ? (
          <div className="py-8 text-center">
            ...
            <h4 className="text-sm font-medium mb-2">No Jellyfin Servers Connected</h4>
            <p className="text-muted-foreground text-sm mb-4 max-w-sm mx-auto">
              Connect your Jellyfin server to sync collections directly to your library. Your collections will appear as Jellyfin collections.
            </p>
            {user?.isAdmin && (
              <Button
                size="sm"
              ...
              <div key={server.id} className="flex items-center justify-between p-4 bg-secondary/30 rounded-lg hover:bg-secondary/50 transition-colors">
                ...
                <div className="flex gap-2">
                  <Button
                    variant="secondary"
                    size="sm"
                    onClick={() => syncServerToJellyfin(server.id)}
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="flex items-center gap-4 text-xs text-muted-foreground mb-2">
                        <span className="flex items-center gap-1">
                          <Server size={12} />
                          {log.jellyfinServerName || 'Unknown Server'}
                        </span>
      ...
      {showAddServer && (
        <AddServerModal
          ...
          onAdded={(server) => {
            setJellyfinServers([...jellyfinServers, server]);
      ...
      {deleteTarget && (
        <ConfirmationModal
          title="Delete Jellyfin Server"
          message={`Are you sure you want to delete "${deleteTarget.name}"? This action cannot be undone.`}
          confirmText="Delete"
          isDangerous
          onConfirm={() => deleteJellyfinServer(deleteTarget.id)}
...
const AddServerModal: FC<AddServerModalProps> = ({ onClose, onAdded }) => {
  ...
  const handleTest = async (): Promise<void> => {
    ...
    try {
      const testResult = await api.testJellyfinConnection(url, apiKey);
  ...
  const handleSubmit = async (e: FormEvent<HTMLFormElement>): Promise<void> => {
    ...
    try {
      ...
      const server = await api.addJellyfinServer({ name, url, apiKey });
      onAdded(server);
    } catch (err: any) {
      setError(err.message);
    } finally {
      ...
      <div className="bg-card border border-border/50 rounded-2xl w-full max-w-md shadow-xl" onClick={(e) => e.stopPropagation()}>
        <div className="flex items-center justify-between p-6 border-b border-border/50">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-lg font-semibold m-0">Add Jellyfin Server</h2>
